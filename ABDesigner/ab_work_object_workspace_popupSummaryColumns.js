...
const ABFieldNumber = require("../AppBuilder/platform/dataFields/ABFieldNumber");
const ABFieldCalculate = require("../AppBuilder/platform/dataFields/ABFieldCalculate");
const ABFieldFormula = require("../AppBuilder/platform/dataFields/ABFieldFormula");

module.exports = class AB_Work_Object_Workspace_PopupSummaryColumns extends ABComponent {
      ...
      var _logic = (this._logic = {
         ...
         clickListItem: function (fieldId) {
            ...
            // unselect
               ...
               SummaryFieldIds = SummaryFieldIds.filter(
                  (fid) => fid != fieldId
               );
         ...
         onShow: function () {
            // refresh list
            var numberFields = CurrentObject.fields(
               (f) => f instanceof ABFieldNumber
            ).map((f) => {
               return {
                  id: f.id,
                  label: f.label,
            ...
            var calculateFields = CurrentObject.fields(
               (f) => f instanceof ABFieldCalculate
            ).map((f) => {
               return {
                  id: f.id,
                  label: f.label,
            ...
            var formulaFields = CurrentObject.fields(
               (f) => f instanceof ABFieldFormula
            ).map((f) => {
               return {
                  id: f.id,
                  label: f.label,
